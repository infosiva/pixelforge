...
// ── Game generation ───────────────────────────────────────────────────────────
const PHASER_SYSTEM = `You are an expert HTML5 game developer using Phaser 3.60.
Generate a COMPLETE, self-contained HTML file with a fully playable game.

REQUIREMENTS:
- Use Phaser 3.60 from CDN: https://cdn.jsdelivr.net/npm/phaser@3.60.0/dist/phaser.min.js
- Full HTML document with <!DOCTYPE html> and all closing tags
- Game canvas must fill the viewport responsively (use scale manager)
- Add keyboard AND touch/mobile controls (tap left/right halves, swipe, virtual buttons as needed)
- Support gamepad/controller via navigator.getGamepads()
- PostMessage bridge at the top for parent iframe: window.addEventListener('message', e => { if(e.data?.type==='GAMEPAD_KEY'){ const ev=new KeyboardEvent(e.data.down?'keydown':'keyup',{key:e.data.key,code:e.data.key,bubbles:true}); window.dispatchEvent(ev); } })
- R key or tap restart button to restart after game over
- Show score/level prominently on screen
- Use procedural graphics (Phaser Graphics API) — no external image assets needed
- Add particle effects, screen shake, and visual polish where appropriate
- Dark, vibrant neon color scheme
- Must have: start screen, gameplay loop, game over screen with score
- IMPORTANT: Return ONLY the complete HTML, no markdown fences, no explanation

Game prompt: `

export async function generateGameCode(prompt: string, genre: string, ageRating: string): Promise<string> {
  const userMsg = `${PHASER_SYSTEM}Genre: ${genre}, Age rating: ${ageRating}\nGame idea: ${prompt}`
  const { text } = await callAI(
    'You generate complete, playable HTML5 Phaser 3 games. Return ONLY the HTML file content.',
...
export async function generateGameMeta(prompt: string, genre: string, ageRating: string): Promise<{ title: string; description: string }> {
  const { text } = await callAI(
    'You generate short game metadata. Return ONLY valid JSON.',
    [{ role: 'user', content: `Game prompt: "${prompt}", genre: ${genre}, age: ${ageRating}.\nReturn JSON: {"title":"Short catchy title (max 4 words)","description":"One engaging sentence describing the game (max 20 words)"}` }],
    200,
    'fast',
  )
  try {
