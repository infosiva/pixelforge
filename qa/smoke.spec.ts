...
test('primary CTA visible', async ({ page }) => { await page.goto('/'); await expect(page.locator('a,button').filter({hasText:/play|build|create|forge|start/i}).first()).toBeVisible({timeout:10_000}); });
